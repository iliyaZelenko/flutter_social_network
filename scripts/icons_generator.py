...
class Icon%s extends StatelessWidget implements IconInterface {
  final double? height;
  final double? width;
  final double? size;
  final Color? color;
  ...
  const Icon%s({Key? key, this.height, this.width, this.size, this.color}) : super(key: key);
...
def gen_markdown():
  ...
  for filename in os.listdir(assetDirectory):
    ...
    line += '`const AppIcon' + var_name.title().replace('_', '') +  "(), `|"  + "\n"
    f1.write("".join(line))
    continue
